...
if (Sector_Config_Area_Type == "infantry") then {
	...
	nul = [base_independent_flagpole,3,true,2,[15,0],50,"default",nil,nil,2] execVM "LV\fillHouse.sqf";
...
if (Sector_Config_Area_Type == "tanks") then {
	...
	nul = [base_independent_flagpole,3,150,[true,false],[true,false,false],false,[15,0],[5,0],"default",nil,nil,2] execVM "LV\militarize.sqf";
};

if (Sector_Config_Area_Type == "motorized") then {
	// Spawn motorized AAF Units
	nul = [base_independent_flagpole,3,150,[true,false],[true,false,false],false,[15,0],[3,0],"default",nil,nil,2] execVM "LV\militarize.sqf";
};

if (Sector_Config_Area_Type == "helicopters") then {
	// Spawn Air AAF Units
	nul = [base_independent_flagpole,3,150,[true,false],[true,false,true],false,[15,0],[5,0],"default",nil,nil,2] execVM "LV\militarize.sqf";
};
	...
	while {true} do {
		...
		if (_aaftickets > 1) then {
			...
			if (_aafforcescount < 5) then {
				...
				nul = [base_independent_flagpole,3,true,false,2000,"random",true,200,50,10,0.2,100,true,false,false,true,["PATROL", getPos base_independent_flagpole, 100],false,"default",nil,nil,1,false] execVM "LV\heliParadrop.sqf";
